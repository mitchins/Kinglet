...
from kinglet.utils import (
    asset_url, media_url, _build_asset_path, _get_cdn_url, 
    _detect_protocol, _get_host
)
...
class TestAssetURLIntegration:
    ...
    def test_media_url_fallback_to_host(self):
        ...
        request.header = Mock(side_effect=lambda h: 'example.com' if h == 'host' else 'https' if h == 'x-forwarded-proto' else None)
