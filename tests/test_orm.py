...
class TestFloatFieldIntegration:
    ...
    async def test_exclude_with_float_fields(self):
        ...
        await self.manager.create(
            self.mock_db, name="Cheap", price=10.0, discount_rate=0.1
        )
        await self.manager.create(
            self.mock_db, name="Expensive", price=100.0, discount_rate=0.2
        )
        await self.manager.create(
            self.mock_db, name="Free", price=0.0, discount_rate=0.0
        )
