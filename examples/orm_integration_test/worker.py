...
from kinglet import (
    Kinglet, 
    Model, StringField, IntegerField, BooleanField, DateTimeField, JSONField,
    SchemaManager, CorsMiddleware
)
...
async def get_schema_sql(request):
    ...
    return {
        "schema_sql": sql,
        "instructions": {
            "wrangler": "npx wrangler d1 execute TEST_DB --command=\"" + sql.replace('"', '\\"') + "\"",
...
async def get_stats(request):
    ...
    try:
        stats = {
            "games": {
                "total": await Game.objects.all(request.env.DB).count(),
                "published": await Game.objects.filter(request.env.DB, is_published=True).count(),
            },
            "users": {
                "total": await User.objects.all(request.env.DB).count(),
                "active": await User.objects.filter(request.env.DB, is_active=True).count(),
            }
        }
...
async def seed_demo_data(request):
    ...
    try:
        ...
        games_data = [
            {"title": "Space Adventure", "description": "Explore the galaxy", "score": 95, "is_published": True, "metadata": {"genre": "adventure", "platform": "web"}},
            {"title": "Puzzle Master", "description": "Mind-bending puzzles", "score": 88, "is_published": True, "metadata": {"genre": "puzzle", "difficulty": "hard"}},
            {"title": "Racing Thunder", "description": "High-speed racing", "score": 92, "is_published": False, "metadata": {"genre": "racing", "multiplayer": True}},
            {"title": "Strategy Empire", "description": "Build your empire", "score": 90, "is_published": True, "metadata": {"genre": "strategy", "turns": "unlimited"}},
            {"title": "Retro Arcade", "description": "Classic arcade games", "score": 85, "is_published": True, "metadata": {"genre": "arcade", "difficulty": "medium"}},
            {"title": "Mystery Quest", "description": "Solve the mystery", "score": 93, "is_published": True, "metadata": {"genre": "mystery", "chapters": 12}},
        ]
        ...
        users_data = [
            {"email": "alice@example.com", "username": "alice_gamer", "profile": {"level": 15, "achievements": ["first_win", "puzzle_master"]}},
            {"email": "bob@example.com", "username": "bob_racer", "profile": {"level": 8, "achievements": ["speed_demon"]}},
            {"email": "carol@example.com", "username": "carol_strategist", "profile": {"level": 22, "achievements": ["empire_builder", "master_tactician"]}},
            {"email": "dave@example.com", "username": "dave_puzzler", "profile": {"level": 12, "achievements": ["brain_teaser"]}},
        ]
        ...
        created_users = []
        if user_instances:
            created_users = await User.objects.bulk_create(request.env.DB, user_instances)
                
        return {
            "success": True,
            "created": {
                "games": len(created_games),
...
async def test_queries(request):
    ...
    try:
        ...
        results["published_games"] = await Game.objects.filter(request.env.DB, is_published=True).count()
        results["high_score_games"] = await Game.objects.filter(request.env.DB, score__gte=90).count()
        results["adventure_games"] = await Game.objects.filter(request.env.DB, title__icontains="adventure").count()
        results["active_users"] = await User.objects.filter(request.env.DB, is_active=True).count()
        
        # Test efficient ordering and limiting
        top_games = await Game.objects.all(request.env.DB).order_by("-score").limit(3).all()
